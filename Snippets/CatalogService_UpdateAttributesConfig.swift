...
func sample(client: CatalogServiceClient, projectId: String, locationId: String, catalogId: String)
  async throws
{
  let response = try await client.updateAttributesConfig(
    request: UpdateAttributesConfigRequest()
      .with {
        $0.attributesConfig = AttributesConfig().with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/catalogs/\(catalogId)/attributesConfig"
        }
