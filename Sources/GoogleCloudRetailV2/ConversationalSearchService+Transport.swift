...
extension Clients {
  final class ConversationalSearchServiceTransport: ConversationalSearchServiceStub {
    ...
    public func conversationalSearch(
      ...
      let (path, query, configure, omitted) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              request.placement as Swift.String?,
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                .literal("/catalogs/"), .singleWildcard, .literal("/placements/"), .singleWildcard,
              ],
              fieldName: "placement")
          else {
            return nil
          }
          let path = "/v2/\(pathVariable0):conversationalSearch"
          let query = [
            URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
          ]
          return (path, query)
        }() {
          return (candidate.0, candidate.1, { $0.setMethod(.POST) }, ["placement"])
        }
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
        ...
        do {
          ...
          builder.maybeAdd(
            ...
            fieldName: "placement",
            expecting: "projects/*/locations/*/catalogs/*/placements/*"
          )
      ...
      return try await req.rpc(
        GoogleCloudRetailV2.ConversationalSearchResponse.self, timeout: options.attemptTimeout
      ).get()
    }
