// Code generated by sidekick. DO NOT EDIT.
//
// Copyright 2026 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     https://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

import Foundation
import GoogleCloudWkt

/// Detailed completion information including completion attribution token and
/// clicked completion info.
public struct CompletionDetail: Codable, Equatable, GoogleCloudWkt._AnyPackable,
  Sendable
{
  /// Completion attribution token in
  /// [CompleteQueryResponse.attribution_token][google.cloud.retail.v2.CompleteQueryResponse.attribution_token].
  ///
  /// [google.cloud.retail.v2.CompleteQueryResponse.attribution_token]: <doc:CompleteQueryResponse/attributionToken>
  public var completionAttributionToken: Swift.String = Swift.String()

  /// End user selected
  /// [CompleteQueryResponse.CompletionResult.suggestion][google.cloud.retail.v2.CompleteQueryResponse.CompletionResult.suggestion].
  ///
  /// [google.cloud.retail.v2.CompleteQueryResponse.CompletionResult.suggestion]: <doc:CompleteQueryResponse/CompletionResult/suggestion>
  public var selectedSuggestion: Swift.String = Swift.String()

  /// End user selected
  /// [CompleteQueryResponse.CompletionResult.suggestion][google.cloud.retail.v2.CompleteQueryResponse.CompletionResult.suggestion]
  /// position, starting from 0.
  ///
  /// [google.cloud.retail.v2.CompleteQueryResponse.CompletionResult.suggestion]: <doc:CompleteQueryResponse/CompletionResult/suggestion>
  public var selectedPosition: Swift.Int32 = Swift.Int32()

  /// Initialize a new instance of `CompletionDetail`.
  public init() {}

  /// Use `config` to return a new instance of this object, with some fields updated.
  ///
  /// Commonly used to initialize the value, for example:
  ///
  /// ```
  /// let value = CompletionDetail().with { $0.completionAttributionToken = ... }
  /// ```
  public func with(_ config: (inout Self) throws -> Swift.Void) rethrows -> Self {
    var copy = self
    try config(&copy)
    return copy
  }

  public static var _anyTypeUrl: String {
    return "type.googleapis.com/google.cloud.retail.v2.CompletionDetail"
  }
  public init(fromAny any: GoogleCloudWkt.`Any`) throws {
    self = try GoogleCloudWkt._slowAnyDeserialize(Self.self, from: any)
  }
  public func _pack() throws -> GoogleCloudWkt.Struct {
    return try GoogleCloudWkt._slowAnySerialize(message: self)
  }
}
