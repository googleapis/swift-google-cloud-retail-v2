...
extension Clients {
  ...
  class ControlServiceTransport: ControlServiceStub {
    ...
    public func createControl(
      ...
      let path = try { () throws -> Swift.String in
        ...
        return "/v2/\(pathVariable0)/controls"
      }()
      var query = [
      ...
      return try GoogleCloudWkt._ProtoJSONDecoder().decode(
        GoogleCloudRetailV2.Control.self, from: data)
    }
    ...
    public func updateControl(
      ...
      let path = try { () throws -> Swift.String in
        guard let pathVariable0 = request.control.map({ $0.name }), !pathVariable0.isEmpty else {
          throw GoogleCloudGax.RequestError.binding("'request.control.name' is not set or is empty")
        }
        return "/v2/\(pathVariable0)"
      }()
      var query = [
    ...
    public func listControls(
      ...
      let (data, _) = try await self.inner.rpc(for: req).get()
      return try GoogleCloudWkt._ProtoJSONDecoder().decode(
        GoogleCloudRetailV2.ListControlsResponse.self, from: data)
    }
